...
在文章开头我就通过上文提到的法一，也就是利用邮件两个头部不对称的问题，伪造了一个米哈游的邮箱，然后通过手动编码修改正文，并加上一个指向我钓鱼网站的链接，这样就成了一封钓鱼邮件。

虽然这样子有的邮件客户端会显示 `由xxx@xxx.com` 代发，但是这样仍然很有欺骗性，因为代发是很常见的，一般人也不会在意。

对此米哈游就没有做到对自己域名邮件足够的防御，但是还是那句话，钓鱼邮件的防御最重要的主体不是发邮件的人，而是看邮件的人。
...
就好像几十年前的人从来没想过几十年后他们的设计依旧是现在互联网重要的基础。而过时的设计往往会导致一些意料之外又情理之中的问题(千年虫？)